...
export default function Step2ClassSelect() {
  const {
    ...
    isLoading, errors, loadClasses, setSelectedClass, clearError,
  } = useTestMaker();
                    ...
                    return (
                      ...
                      >
                        <i
                          ...
                          style={{ fontSize: '22px', color: sel ? 'white' : c.icon, marginBottom: '6px' }}
                        />
                        <span
                          className="s2-tile-label"
                          style={{ color: sel ? 'white' : c.icon }}
                        >
                          {cls.class_name}
                        </span>
                        {sel && (
                          <div className="s2-tile-check">
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
          .s2-breadcrumb { display: none; }
          .s2-header { margin-top: 72px; } /* no breadcrumb, so header needs the top gap */
          .s2-page { padding: 24px 16px 40px; }
        }

        /* ── Mobile (≤480px) ── */
        @media (max-width: 480px) {
          .s2-page { padding: 16px 12px 32px; }
          .s2-header { margin-top: 68px; margin-bottom: 20px; }
          .s2-num { width: 40px; height: 40px; font-size: 16px; }
          .s2-title { font-size: 22px; gap: 8px; }
          .s2-subtitle { font-size: 13px; padding: 0 8px; }
          .s2-card { padding: 18px 14px; border-radius: 16px; }
          .s2-tile-grid { grid-template-columns: repeat(auto-fill, minmax(76px, 1fr)); gap: 8px; }
          .s2-tile { min-height: 72px; padding: 12px 8px; }
          .s2-tile-label { font-size: 11px; }
          .s2-sel-name { font-size: 14px; }
          .s2-btn-back { padding: 13px 32px; width: 100%; justify-content: center; }
        }

        /* ── When breadcrumb is visible, header needs less top margin ── */
        @media (min-width: 769px) {
          .s2-header { margin-top: 0; }
        }
      `}</style>
    </div>
  );
