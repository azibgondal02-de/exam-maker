...
export default function Step3SubjectSelect() {
  const {
    ...
    isLoading, errors, loadSubjects, setSelectedSubject, clearError,
  } = useTestMaker();
        ...
        {isLoading ? (
          ...
          <>
            {subjects.length > 0 ? (
              <>
                {newSubjects.length > 0 && (
                  <Section title="Latest" badge="s3-badge-new" subjects={newSubjects} offset={0} />
                )}
                {oldSubjects.length > 0 && (
                  <Section title="Previous" badge="s3-badge-old" subjects={oldSubjects} offset={newSubjects.length} />
                )}
              </>
            ) : (
            ...
            <div className="s3-actions">
              <button
                onClick={() => window.location.href = '/test-maker/step-2'}
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
          .s3-breadcrumb { display: none; }
          .s3-header { margin-top: 72px; }
          .s3-page { padding: 24px 16px 40px; }
          .s3-subj-grid { grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); }
        }

        /* ── Mobile (≤480px) ── */
        @media (max-width: 480px) {
          .s3-page { padding: 16px 12px 32px; }
          .s3-header { margin-top: 68px; margin-bottom: 20px; }
          .s3-num { width: 40px; height: 40px; font-size: 16px; }
          .s3-title { font-size: 22px; gap: 8px; }
          .s3-subtitle { font-size: 13px; padding: 0 8px; }
          .s3-section { padding: 16px 12px; border-radius: 16px; }
          .s3-subj-grid { grid-template-columns: repeat(auto-fill, minmax(88px, 1fr)); gap: 8px; }
          .s3-subj-btn { min-height: 84px; padding: 12px 8px; }
          .s3-btn-back { padding: 13px 32px; width: 100%; justify-content: center; }
        }

        /* ── Desktop: no extra top margin since breadcrumb provides the gap ── */
        @media (min-width: 769px) {
          .s3-header { margin-top: 0; }
        }
      `}</style>
    </div>
  );
